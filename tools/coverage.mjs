...
// FAQ 문항 수. **실측이 있으면 실측을 쓴다.**
//
// 🔴 2026-08-19: 이 표는 원래 docs/04에서 옮겨 적은 값이었는데, 제목을 실제로 수집해보니
//    카드가 337이 아니라 358이었다(투자도 164→165). 옮겨 적은 값이 낡아 있었던 것이다.
//    → 수집한 카테고리는 data/faq-titles.json 에서 **세어서** 쓰고,
//      아직 수집 못 한 카테고리만 docs/04 추정치를 쓴다.
const 미수집 = {   // ⚠️ docs/04 (2026-08-17 집계). 실측 아님 — 낡았을 수 있다
  '사업자': 188, '제휴서비스': 150, '외환': 146, 'mini': 142, '인증/보안': 115,
  '기타': 113, 'AI': 72, '앱 이용': 83, 'ATM': 31, '개인금고': 19, '알림': 18,
};
const 실측 = fs.existsSync('data/faq-titles.json')
  ? Object.fromEntries(Object.entries(JSON.parse(fs.readFileSync('data/faq-titles.json', 'utf8')))
      .map(([k, v]) => [k === '통장저축' ? '통장/저축' : k, v.length]))
  : {};
const FAQ = { ...미수집, ...실측 };
